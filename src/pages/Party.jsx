...
const Party = () => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      ...
      () => {
        // หากดึงพิกัดไม่ได้ ให้โหลดข้อมูลแบบไม่มีระยะทาง
        setLoading(false);
        loadData();
      },
      { enableHighAccuracy: true, timeout: 5000 },
    );
  }, []); // 🌟 รันเฉพาะตอนเข้าหน้าครั้งแรกพอ
            ...
            <div className="flex flex-col gap-3 pt-2 mb-4">
              ...
              <div className="flex gap-4 overflow-x-auto no-scrollbar py-2 px-1">
                {myJoinedGroups.map((party) => {
                  ...
                  const imageUrl = isQuickBill ? "https://ui-avatars.com/api/?name=Bill&background=F7EAD7&color=A65D2E&bold=true" : (restaurant.images?.find((img) => img.isCover)?.url || restaurant.images?.[0]?.url || "https://picsum.photos/seed/restaurant/400/300");
