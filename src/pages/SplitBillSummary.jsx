...
const SplitBillSummary = () => {
    ...
    const [settingsForm, setSettingsForm] = useState({ vat: 0, serviceCharge: 0 });
    ...
    const handleUpdateSettings = async () => {
        ...
        try {
            ...
            await loadData();
            toast.success("อัปเดตข้อมูลบิลสำเร็จ");
        } catch (error) {
    ...
    if (loading && !billSummary) return (
        <div className="w-full min-h-screen bg-base-100 flex justify-center items-center">
    ...
    return (
        <div className="relative w-full h-screen bg-base-100 text-base-content font-sans overflow-hidden flex flex-col">
            <header className="bg-base-100/90 backdrop-blur-xl px-6 pt-12 pb-4 flex items-center gap-4 shadow-sm z-40 shrink-0">
                <button onClick={() => navigate(-1)} className="p-2 -ml-2 rounded-full hover:bg-[#F7EAD7] transition-colors"><ArrowLeft size={24} className="text-[#2B361B]" /></button>
            ...
            <main className="flex-1 overflow-y-auto no-scrollbar px-6 pt-6 pb-48">
                ...
                {isCompleted && (
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="mb-8 p-5 rounded-[2rem] bg-secondary text-white shadow-xl relative overflow-hidden">
                        ...
                        <div className="flex items-center gap-4 relative z-10">
                            ...
                            <button 
                                ...
                                className={`px-5 py-2.5 rounded-full text-[11px] font-black transition-all ${hasReviewed ? 'bg-white/10 text-white/40' : 'bg-accent text-white shadow-lg active:scale-95'}`}
                            >
                ...
                <div className="bg-secondary rounded-[2rem] p-6 mb-8 text-white relative overflow-hidden shadow-xl">
                ...
                {isLeader && (
                    <div className="bg-white border border-[#EEE2D1] rounded-[1.5rem] p-5 mb-8 shadow-sm">
                        <h3 className="text-[10px] font-bold text-[#8B837E] uppercase tracking-widest mb-4 flex items-center gap-2"><Receipt size={14} className="text-[#A65D2E]" /> ตั้งค่าค่าใช้จ่ายเพิ่มเติม (โดยหัวหน้า)</h3>
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="text-[9px] font-black text-[#A65D2E] uppercase block mb-1 ml-1">Service Charge (฿)</label><input type="number" value={settingsForm.serviceCharge} onChange={(e) => setSettingsForm({ ...settingsForm, serviceCharge: e.target.value })} onBlur={handleUpdateSettings} disabled={isCompleted || actionLoading} placeholder="0" className="w-full bg-base-100 border border-[#EEE2D1] rounded-xl py-2 px-3 text-sm font-bold outline-none focus:border-[#A65D2E] transition-all disabled:opacity-50" /></div>
                            <div><label className="text-[9px] font-black text-[#A65D2E] uppercase block mb-1 ml-1">VAT (฿)</label><input type="number" value={settingsForm.vat} onChange={(e) => setSettingsForm({ ...settingsForm, vat: e.target.value })} onBlur={handleUpdateSettings} disabled={isCompleted || actionLoading} placeholder="0" className="w-full bg-base-100 border border-[#EEE2D1] rounded-xl py-2 px-3 text-sm font-bold outline-none focus:border-[#A65D2E] transition-all disabled:opacity-50" /></div>
                        </div>
                        {!isCompleted && <p className="text-[8px] text-gray-400 mt-3 italic text-center">* ยอดเงินที่กรอกจะถูกหารเฉลี่ยให้สมาชิกทุกคนในปาร์ตี้อัตโนมัติ</p>}
                    </div>
                )}
