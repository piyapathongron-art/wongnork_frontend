...
const SplitBillSummary = () => {
  ...
  if (loading && !billSummary)
    return (
      <div className="w-full min-h-screen bg-[#FFF8F5] flex justify-center items-center">
        <span className="text-[#A65D2E] font-bold animate-pulse tracking-widest">
          LOADING...
        </span>
      </div>
    );

  const mySummary = billSummary?.members?.find(
    (m) => m.user.id === user?.id,
  ) || {
    items: [],
  ...
  return (
    <div className="relative w-full h-screen bg-[#FFF8F5] text-[#2B361B] font-sans overflow-hidden flex flex-col">
      <header className="bg-[#FFF8F5]/90 backdrop-blur-xl px-6 pt-12 pb-4 flex items-center gap-4 shadow-sm z-40 shrink-0">
        <button
          onClick={() => navigate(-1)}
          className="p-2 -ml-2 rounded-full hover:bg-[#F7EAD7] transition-colors"
        >
          <ArrowLeft size={24} className="text-[#2B361B]" />
        </button>
        <div className="flex-1 overflow-hidden">
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-extrabold text-[#2B361B] tracking-tight leading-none">
              สรุปบิลรวม
            </h1>
            {isCompleted && (
          ...
          <p className="text-[11px] font-bold text-[#A65D2E] uppercase tracking-wider mt-1 truncate">
            {party?.name}
          </p>
        </div>
        <PartyControlMenu
      ...
      >
        ...
        {isLeader && (
          <div className="bg-white border border-[#EEE2D1] rounded-[1.5rem] p-5 mb-8 shadow-sm">
            <h3 className="text-[10px] font-bold text-[#8B837E] uppercase tracking-widest mb-4 flex items-center gap-2">
              <Receipt size={14} className="text-[#A65D2E]" />{" "}
              ตั้งค่าค่าใช้จ่ายเพิ่มเติม (โดยหัวหน้า)
            </h3>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-[9px] font-black text-[#A65D2E] uppercase block mb-1 ml-1">
                  Service Charge (฿)
                </label>
                <input
                  type="number"
                  ...
                  placeholder="0"
                  className="w-full bg-[#FFF8F5] border border-[#EEE2D1] rounded-xl py-2 px-3 text-sm font-bold outline-none focus:border-[#A65D2E] transition-all disabled:opacity-50"
                />
              </div>
              <div>
                <label className="text-[9px] font-black text-[#A65D2E] uppercase block mb-1 ml-1">
                ...
                <input
                  type="number"
                  value={settingsForm.vat}
                  onChange={(e) =>
                    setSettingsForm({ ...settingsForm, vat: e.target.value })
                  }
                  onBlur={handleUpdateSettings}
                  disabled={isCompleted || actionLoading}
                  placeholder="0"
            ...
            {!isCompleted && (
              <p className="text-[8px] text-gray-400 mt-3 italic text-center">
                * ยอดเงินที่กรอกจะถูกหารเฉลี่ยให้สมาชิกทุกคนในปาร์ตี้อัตโนมัติ
              </p>
            )}
          </div>
        )}
        ...
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-[12px] font-bold text-[#8B837E] uppercase tracking-[0.2em]">
            รายการอาหารในโต๊ะ
          </h3>
          {!isCompleted && (
            <button
              onClick={() => setIsAddCustomModalOpen(true)}
              className="text-[#A65D2E] text-xs font-bold flex items-center gap-1 hover:underline"
            >
                ...
                >
                  <div className="flex gap-3 mb-3">
                    {item.imageUrl ? (
                      <img
                        src={item.imageUrl}
                        alt={item.name}
                        className="w-12 h-12 rounded-lg object-cover shrink-0 border border-[#EEE2D1]"
                      />
                    ) : (
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-[14px] text-[#2B361B] truncate">
                        {item.name}{" "}
                        {item.isCustom && (
                          <span className="text-[10px] bg-gray-100 px-1.5 py-0.5 rounded text-gray-500 font-medium ml-1">
                            พิเศษ
                          </span>
                        )}
                      </h4>
                      <div className="text-[12px] font-black text-[#A65D2E]">
                  ...
                  <div className="flex items-center justify-between border-t border-[#EEE2D1]/50 pt-3">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {isOptIn ? (
                          <>
                            <Check size={12} strokeWidth={3} /> คุณร่วมหาร
                          </>
                        ) : (
                          "ฉันไม่ได้กิน"
                        )}
                        ...
                        <div className="flex -space-x-1">
                          {item.sharers.slice(0, 3).map((s) => (
                            <img
                              ...
                              alt={s.name}
                              className="w-6 h-6 rounded-full border-2 border-white bg-gray-200 object-cover shadow-sm"
                              title={s.name}
                            />
                          ))}
                          {item.sharers.length > 3 && (
                            <div className="w-6 h-6 rounded-full border-2 border-white bg-gray-100 flex items-center justify-center text-[8px] font-bold text-gray-600 shadow-sm">
                              +{item.sharers.length - 3}
                            </div>
                          )}
                        </div>
                    ...
                    <div className="flex items-center gap-3 bg-[#FFF8F5] p-1 rounded-xl border border-[#EEE2D1]">
                      <button
                        ...
                        className="w-7 h-7 flex items-center justify-center rounded-lg bg-white shadow-sm text-red-500 hover:bg-red-50 active:scale-95 transition-colors disabled:opacity-30"
                      >
                        {item.quantity === 1 ? (
                          <Trash2 size={14} />
                        ) : (
                      ...
                      <span className="font-bold text-[14px] w-4 text-center text-[#2B361B]">
                        {item.quantity}
                      </span>
                      <button
                        onClick={() =>
                        ...
                        disabled={actionLoading || isCompleted}
                        className="w-7 h-7 flex items-center justify-center rounded-lg bg-[#2B361B] shadow-sm text-white hover:bg-[#1A2210] active:scale-95 transition-colors disabled:opacity-30"
                      >
                        <Plus size={14} />
                      </button>
                    </div>
                  ...
                  {isOptIn && item.costPerPerson > 0 && (
                    <div className="mt-3 bg-[#FFF8F5] rounded-xl p-2.5 flex justify-between items-center border border-[#EEE2D1]">
                      <span className="text-[10px] font-bold text-[#8B837E]">
                        คุณหารอยู่ที่:
                      </span>
                      <span className="text-[13px] font-black text-[#A65D2E]">
                        ฿{Math.ceil(item.costPerPerson).toLocaleString()}
                      </span>
                    </div>
                  )}
                </motion.div>
        ...
        {itemToDelete && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-xs rounded-[2rem] p-6 text-center shadow-2xl"
            >
              <div className="w-16 h-16 bg-red-100 text-red-500 rounded-full flex items-center justify-center mx-auto mb-4">
                <Trash2 size={24} />
              </div>
              <h3 className="text-lg font-bold text-[#2B361B] mb-2">
                ลบรายการอาหาร?
              </h3>
              <p className="text-sm text-[#8B837E] mb-6">
                คุณแน่ใจหรือไม่ว่าต้องการลบ "{itemToDelete.name}" ออกจากบิลโต๊ะ
              </p>
              <div className="flex gap-3">
                <button
                  onClick={() => setItemToDelete(null)}
                  className="flex-1 py-3 bg-gray-100 text-[#8B837E] hover:bg-gray-200 transition-colors rounded-xl font-bold text-sm"
                >
                  ยกเลิก
                </button>
                <button
                  onClick={handleDeleteItem}
                  className="flex-1 py-3 bg-red-500 hover:bg-red-600 transition-colors text-white rounded-xl font-bold text-sm shadow-md"
                >
                  ลบรายการ
                </button>
              </div>
        ...
        {isAddCustomModalOpen && (
          ...
          >
            <motion.div
              initial={{ scale: 0.9, opacity: 0, y: 20 }}
              animate={{ scale: 1, opacity: 1, y: 0 }}
              className="bg-[#FFF8F5] w-full max-w-sm rounded-[2.5rem] p-8 shadow-2xl border border-[#EEE2D1]"
            >
              <h3 className="text-xl font-bold text-[#2B361B] mb-6">
                เพิ่มเมนูพิเศษ 🍳
              </h3>
              <div className="space-y-4 mb-8">
                <div>
                  <label className="text-[10px] font-bold text-[#8B837E] uppercase tracking-widest block mb-2 ml-1">
                    ชื่อเมนู
                  </label>
                  <input
                    type="text"
                    ...
                    placeholder="เช่น เครื่องดื่ม, ค่าเปิดขวด"
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 bg-[#A65D2E] hover:bg-[#8B4D24] transition-colors text-white py-3 rounded-xl text-sm font-bold shadow-md"
                >
                  {actionLoading ? "กำลังเพิ่ม..." : "เพิ่มลงบิล"}
                </button>
              </div>
            </motion.div>
          </motion.div>
        ...
        {isCompleteModalOpen && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-xl font-black text-[#2B361B] mb-3">
                ยืนยันการสรุปยอดบิล?
              </h3>
              <p className="text-sm text-[#8B837E] mb-8 leading-relaxed">
                เมื่อยืนยันแล้ว
                สมาชิกทุกคนจะไม่สามารถแก้ไขรายการอาหารหรือจำนวนได้อีก
                คุณแน่ใจใช่หรือไม่?
              </p>
              <div className="flex gap-3">
                <button
                ...
                <button
                  ...
                  disabled={actionLoading}
                  className="flex-1 py-3 bg-green-600 hover:bg-green-700 transition-colors text-white rounded-xl font-bold text-sm shadow-md"
                >
                  ใช่, ปิดจ็อบเลย
                </button>
              </div>
