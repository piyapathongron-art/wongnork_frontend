  ...
  const handleMarkerClick = async (restaurantData) => {
    ...
    try {
      ...
      setSelectedRestaurant(prev => ({
        ...
        menu: menusRes.data.data,
      }));
    } catch (error) { console.error("Details fetch error", error); }
  };

  return (
    <div className="w-full h-screen relative overflow-hidden bg-white dark:bg-black touch-none font-sans">
      ...
      {/* FLOATING OWNER ACTION */}
      {user?.role === "OWNER" && (
        <button 
          onClick={() => setIsCreateModalOpen(true)}
          className="fixed top-24 right-6 z-40 w-16 h-16 bg-[#BC6C25] text-[#F7EAD7] rounded-full shadow-2xl flex flex-col items-center justify-center border-4 border-white dark:border-zinc-900 active:scale-90 transition-transform "
        >
          <Store className="w-6 h-6" />
          <span className="text-[8px] font-black uppercase mt-0.5">Add Shop</span>
