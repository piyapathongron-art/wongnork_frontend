...
const CreatePartyModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    useEffect(() => {
        if (selectedRestaurantId && mapRef.current) {
            const target = filteredRestaurants.find(r => String(r.id) === String(selectedRestaurantId));
            if (target) {
            ...
            <div className="bg-base-200 w-full max-w-lg rounded-[40px] overflow-hidden shadow-2xl border border-primary/20 flex flex-col max-h-[92vh]">
                ...
                <div className="p-8 overflow-y-auto no-scrollbar pb-32">
                    ...
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 text-left">
                        {/* Location Dropdown */}
                        <div>
                            <label className="block text-[10px] font-black uppercase text-primary mb-1 ml-2">Meeting Point</label>
                            <select 
                            ...
                            {errors.restaurantId && <p className="text-red-500 text-[10px] mt-1 ml-2 font-bold uppercase">{errors.restaurantId.message}</p>}
                        </div>

                        {/* Party Name */}
                        <div>
                            <label className="block text-[10px] font-black uppercase text-primary mb-1 ml-2">Party Name</label>
                            <input 
                                {...register("name")}
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            {/* Max Participants */}
                            <div>
                                <label className="block text-[10px] font-black uppercase text-primary mb-1 ml-2">Max Slots</label>
                                <input 
                                    type="number"
                            ...
                            {/* Meetup Time */}
                            <div>
                                <label className="block text-[10px] font-black uppercase text-primary mb-1 ml-2">Time</label>
                                <input 
                                    type="datetime-local"
                                    {...register("meetupTime")}
                                    className="w-full bg-white/50 border-2 border-base-300  rounded-2xl px-4 py-3 text-sm"
                        ...
                        {/* Contact Info */}
                        <div>
                            <label className="block text-[10px] font-black uppercase text-primary mb-1 ml-2">Contact Info</label>
                            <input 
                        ...
                        >
                            {isSubmitting ? <Loader2 className="animate-spin w-5 h-5" /> : "Start the Party"}
                        </button>
                    </form>
                </div>
            </div>
