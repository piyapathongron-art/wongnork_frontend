...
const NavBar = () => {
    ...
    const getColors = (id) => ({
        ...
        opacity: activeTab === id ? "opacity-100 scale-110" : "opacity-50 hover:opacity-80",
        ...
        <div className="fixed bottom-6 left-0 right-0 flex justify-center px-4 z-50 pointer-events-none">
            <nav className="w-full max-w-[402px] bg-[#F7EAD7] h-[100px] rounded-[50px] flex items-center justify-around px-4 shadow-lg border border-[#EEE2D1]/30 pointer-events-auto">
                ...
                >
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-tighter text-center leading-[0.7rem] ${getColors('ai-recommend').text}`}>
                        Talk with<br />WongNork
                    </span>
                    {activeTab === 'ai-recommend' && <div className="w-1.5 h-1.5 bg-[#BC6C25] rounded-full mt-0.5" />}
                </div>
                ...
                >
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-tighter ${getColors('location').text}`}>Location</span>
                    {activeTab === 'location' && <div className="w-1.5 h-1.5 bg-[#BC6C25] rounded-full mt-0.5" />}
                </div>
                ...
                >
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-tighter ${getColors('my-parties').text}`}>Create Group</span>
                    {activeTab === 'my-parties' && <div className="w-1.5 h-1.5 bg-[#BC6C25] rounded-full mt-0.5" />}
                </div>
                ...
                >
                    ...
                    <span className={`text-[10px] font-bold uppercase tracking-tighter ${getColors('profile').text}`}>Profile</span>
                    {activeTab === 'profile' && <div className="w-1.5 h-1.5 bg-[#BC6C25] rounded-full mt-0.5" />}
                </div>
